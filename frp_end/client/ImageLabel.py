...
class ImageLabel(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        # 【修改】关闭 setScaledContents，因为我们用 paintEvent 自己绘制
    ...
    def set_pixmap_from_data(self, image_data: QByteArray):
        ...
        # 【修改】加载成功后，清除文字和 setPixmap，然后调用 update() 触发 paintEvent
        self.setText("")
    ...
    def resizeEvent(self, event: QResizeEvent):
        """覆盖 resizeEvent 方法 (逻辑不变，但现在它会触发 paintEvent)"""
    ...
    def mousePressEvent(self, event):
        """覆盖 mousePressEvent 来发射点击信号 (逻辑不变)"""
        if event.button() == Qt.LeftButton:
            self.clicked.emit()
        super().mousePressEvent(event)

    def setMovie(self, movie: QMovie):
        """覆盖 setMovie 方法，确保清理静态图并统一光标行为"""
        self._original_pixmap = QPixmap() # 播放动画时，清除静态图
        
        # 【修改】在设置动画时，也应用统一的圆角背景样式
