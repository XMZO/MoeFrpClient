...
from argon2.exceptions import VerifyMismatchError # 【新增】用于捕获密码不匹配的异常
import logging
...
# Argon2id fbe04fbf8e52b85747e811e71b7d3331d6b722db7a6aeef615d3ec3da86bd15bjkiyiuuio1145141919810721-版本号  WOOD0721 10并行 1024MB 20迭代 100哈希 16进制
# 【核心大改】最终的数据结构，绑定所有信息
# 这是一个列表，每个元素代表一个受信任的版本
TRUSTED_CLIENTS = [
    {
        "version": "105",
        "secret": "db0b85ade67d530c4ede48b75438e21b4f45dcf27d9397d80ce437121295191d250c1a96567d24dda341915dfded2a0af210e72d4f40fb44d3358e6f383b6f50416db50662c42fdf7a8eae81699a51b7842598a728dc050334c4f4e4c55016527aa04611c",
        # 假设v103的dll哈希和v101一样
        "dll_hash": "29126d5a2dd11e9d9483adcd871bc9a9559d669f011777034415ea4fe9054790" 
    },
    {
        "version": "114514",
        "secret": "$argon2id$v=19$m=100000,t=20,p=10$bG9saWxvbGk$dc4K33OREII0AR0troSUvbgKFNBehW3bdsbpYY34NuRvJ0zoOEPsbTtHtuBkeleI/DSnaKs0GmBOr7O+X1w2IMnXWScflssGH6BtO+jpyB2ecmEl4d65uf3tiaFdAsEz1YFGEg",
        # 假设v103的dll哈希和v101一样
        "dll_hash": "imwood" 
    },
]
TRUSTED_SECRET_MAP = {client['secret']: client for client in TRUSTED_CLIENTS}
MIN_CLIENT_VERSION = 105
LATEST_CLIENT_VERSION = 105
LATEST_CLIENT_VERSION_STR = "v1.0.3c"
...
def initiate_password_reset(user): # user 对象由装饰器传入
    ...
    with reset_tokens_lock:
        # (这部分逻辑需要修改，因为它现在不直接操作数据库了)
        # 应该有一个函数来创建重置令牌并存入数据库
        # 为简化，我们直接在这里写
        db = get_db()
        # 先删除可能存在的旧令牌
...
def set_new_session(db, user_uuid):
    ...
    # 【修改】使用 now(timezone.utc) 来创建带时区信息的时间
...
def get_temp_config(config_id):
    ...
    with configs_lock:
        ...
        if config_data['uses_left'] > 0:
            ...
            # 【修改】如果是第一次使用，记录下 `first_use_time`
